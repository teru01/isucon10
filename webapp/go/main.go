...
func main() {
	...
	estateMySQLConnectionData := estateNewMySQLConnectionEnv()
	chairMySQLConnectionData := chairNewMySQLConnectionEnv()
...
>>>>>>> origin/sharding
	...
	estate.SetMaxOpenConns(10)
	defer estate.Close()

	chair, err := chairMySQLConnectionData.ConnectDB()
	if err != nil {
		e.Logger.Fatalf("chair DB connection failed : %v", err)
	}
...
func postChair(c echo.Context) error {
  ...
  tx, err := chairDB.Begin() if err != nil {
...
func searchChairs(c echo.Context) error { conditions := make([]string, 0)
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	chair := Chair{}
	query := `SELECT * FROM chair WHERE id = ?`
	err = estateDB.Get(&chair, query, id)
	if err != nil {
		if err == sql.ErrNoRows {
